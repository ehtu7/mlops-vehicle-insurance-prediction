    ...
    def __init__(self, database_name: str = DATABASE_NAME) -> None:
        ...
        try:
            ...
            if MongoDBClient.client is None:
                # Ensure the environment variable key is set, not the URL itself
                # mongo_db_url = os.getenv(MONGODB_URL_KEY)
                mongo_db_url='mongodb+srv://emazhar:emazhar@mlops-proj.u0vso5a.mongodb.net/'
                logging.info(f"MongoDB URL from env: {mongo_db_url}")
                if not mongo_db_url:
                    raise Exception(
                        f"Environment variable '{MONGODB_URL_KEY}' is not set. Please set it to your MongoDB connection string."
                    )
